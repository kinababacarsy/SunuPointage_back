...
exports.getControleAccesByUserId = async(req, res) => {
    try {
        const controlesAcces = await ControleAcces.find({ userId: req.params.userId });
        ...
        return res.status(500).json({ message: 'Erreur lors de la récupération des pointages', error: error.message });
    }
};