...
// MongoDB connection
mongoose
    .connect("mongodb://localhost:27017/sunupointage")
...
const port = new SerialPort({
    path: "COM6", // Port série utilisé par Arduino
    baudRate: 9600,
    autoOpen: false,
});
    ...
    if (data.includes("UID de la carte lue :")) {
        ...
        try {
            ...
            if (user) {
                ...
                const pointages = await ControleAcces.find({
                    userId: user.id,
                    date: formattedDate,
                }).sort({ heure: 1 });
                ...
                if (pointages.length === 0) {
                    ...
                    console.log(
                        "Dernier pointage de la journée :",
                        dernierPointage
                    );
                }
                ...
                if (!existingCheckIn) {
                    ...
                    if (existingCheckOut) {
                        ...
                        existingCheckOut.etat =
                            hourStr < "17:00" ? "Présent" : "Absent"; // Définir l'état
...
app.get("/controle-acces/pointages/:cardID", async (req, res) => {
    ...
    try {
        ...
        const pointages = await ControleAcces.find({ userId: user.id }).sort({
            date: 1,
            heure: 1,
        });
...
async function getPointagesOfTheDay() {
    try {
        ...
        const pointages = await ControleAcces.find({ date: dateStr }).sort({
            heure: 1,
        });
        ...
        console.error(
            "Erreur lors de la récupération des pointages de la journée:",
            error
        );
        throw error;
    }
