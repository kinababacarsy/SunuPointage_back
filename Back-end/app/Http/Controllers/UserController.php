...
class UserController extends Controller
{
    ...
    public function createFromDepartement(Request $request, $departement_id)
    {
        ...
        // Vérifier si le rôle est fourni dans la requête
        if (!$request->has('role')) {
            return response()->json(['message' => 'Le rôle est requis pour créer un utilisateur dans un département'], 400);
        }

        // Définir le département_id
        $request->merge(['departement_id' => $departement_id]);
